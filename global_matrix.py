...
def create(N, c, f, BC, A0, A1, A2):
	...
	A0_ = 	[ A0[2,2], A0[2,3], A0[3,2], A0[3,3] ] + \
			(N-2)*[ i for a in A0 for i in a ] + \
			[ A0[0,0], A0[0,1], A0[1,0], A0[1,1] ]
	
	A1_ = 	[ A1[2,2], A1[2,3], A1[3,2], A1[3,3] ] + \
			(N-2)*[ i for a in A1 for i in a ] + \
			[ A1[0,0], A1[0,1], A1[1,0], A1[1,1] ]
	
	A2_ = 	[ A2[2,2], A2[2,3], A2[3,2], A2[3,3] ] + \
	...
	row = 	[0, 0, 1, 1] + \
			[ i  for j in range(2, 2*N-2, 2) for i in range(j-2, j+2)  for k in range(4) ] + \
			[2*N-4, 2*N-4, 2*N-3, 2*N-3]
	
	col = 	[0, 1, 0, 1] + \
			[ i  for j in range(2, 2*N-2, 2) for k in range(4)  for i in range(j-2, j+2) ] + \
			[2*N-4, 2*N-3, 2*N-4, 2*N-3]
